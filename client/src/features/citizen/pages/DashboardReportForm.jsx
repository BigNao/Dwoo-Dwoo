...
const STORAGE_KEY = "dwoo_citizen_report_draft";
...
export default function DashboardReportForm({ onClose }) {
  ...
  useEffect(() => {
    ...
    const data = { step, incidentType, description, position, locationConfirmed };
    saveDraft(data);
  }, [step, incidentType, description, position, locationConfirmed, confirmation]);
  ...
  if (confirmation) {
    return <ConfirmationScreen confirmation={confirmation} navigate={navigate} onClose={onClose} />;
  }
    ...
    <div className="space-y-6">
      ...
      {draftRestored && (
        <div className="mb-6 flex items-center justify-between rounded-sign border border-accent/30 bg-accent/5 px-4 py-3">
          <p className="text-sm text-ink dark:text-white">Draft restored from your last session.</p>
          <button
            type="button"
  ...
  return (
    <div>
      <label className="block text-sm font-medium mb-2 text-ink dark:text-white">
        Photo or Video <span className="text-muted/70 dark:text-white/40 font-normal">(optional)</span>
      </label>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-center gap-2">
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-semibold transition-colors ${cameraMode === "photo" ? "bg-primary dark:bg-accent text-white dark:text-ink" : "bg-muted/20 text-ink dark:text-white hover:bg-muted/40"}`}
            >
              Photo
            </button>
            <button
          ...
          <div className="flex gap-3">
            {cameraMode === "photo" ? (
              <button
                ...
                className="flex-1 px-4 py-2.5 rounded-lg bg-primary dark:bg-accent text-white dark:text-ink text-sm font-semibold hover:bg-primary-hover dark:hover:bg-accent-dark transition-colors disabled:opacity-50"
              >
                Capture Photo
              </button>
            ) : recording ? (
...
function ConfirmationScreen({ confirmation, navigate, onClose }) {
  return (
