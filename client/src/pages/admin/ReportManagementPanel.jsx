...
export default function ReportManagementPanel({ report, onClose, onUpdated }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-primary/40" onClick={onClose}>
      <div
        className="h-full w-full max-w-md bg-background text-ink overflow-y-auto shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="sticky top-0 bg-background border-b border-border px-6 py-4 flex items-center justify-between">
          <div>
            <p className="text-xs font-mono text-muted/80">{report.reference_number}</p>
            <h2 className="font-display text-lg font-semibold">{report.incident_type}</h2>
          </div>
          <button type="button" onClick={onClose} className="text-2xl leading-none text-muted/70 hover:text-ink">
            ×
          </button>
        </div>
        ...
        <div className="px-6 py-5 space-y-6">
          <div className="flex items-center gap-3">
            ...
            {report.corroboration_flag && (
              <span className="sign-badge text-secondary bg-secondary-light">Corroborated</span>
            )}
          </div>

          <div>
            <p className="text-xs font-mono uppercase tracking-wide text-muted/80 mb-1">Description</p>
          ...
          <dl className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <dt className="text-xs text-muted/80 uppercase tracking-wide mb-1">Location</dt>
          ...
          <div className="border-t border-border pt-5">
            ...
            <select
              ...
              className="w-full rounded-sign border border-border px-4 py-2.5 bg-card"
            >
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full rounded-sign border border-border px-4 py-3 bg-card resize-none"
            />
          </div>

          {saveError && <p className="text-sm text-danger font-medium">{saveError}</p>}
          ...
          <div className="border-t border-border pt-5">
            <p className="text-xs font-mono uppercase tracking-wide text-muted/80 mb-3">Audit trail</p>

            {logsLoading && <p className="text-sm text-muted/70">Loading history…</p>}

            {!logsLoading && logs.length === 0 && (
              <p className="text-sm text-muted/70">No status changes recorded yet.</p>
            )}

            <ul className="space-y-3">
              {logs.map((log) => (
                <li key={log.log_id} className="text-sm border-l-2 border-accent pl-3">
                  <p className="font-medium">
                    {STATUS_LABELS[log.previous_status] || log.previous_status} →{" "}
                    {STATUS_LABELS[log.new_status] || log.new_status}
                  </p>
                  <p className="text-xs text-muted/80">{formatTimestamp(log.action_timestamp)}</p>
                  {log.admin_notes && <p className="text-xs text-muted mt-1">{log.admin_notes}</p>}
                </li>
              ))}
            </ul>
          </div>
